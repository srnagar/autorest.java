 ...
 * data. It supports two kinds of mode, one is for stateless using, another is
 * for stateful using. In stateless mode, there are three functionalities.
 ...
 * series. In stateful mode, user can store time series, the stored time series
 * will be used for detection anomalies. Under this mode, user can still use
 * the above three functionalities by only giving a time range without
 * preparing time series in client side. Besides the above three
 * functionalities, stateful model also provide group based detection and
 * labeling service. By leveraging labeling service user can provide labels for
 * each detection result, these labels will be used for retuning or
